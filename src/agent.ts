...
export async function setupAgent(restConfig: AriesRestConfig) {
  ...
  let publicServerBinding: Promise<void> | undefined
  for (const inboundTransport of inboundTransports) {
    if (inboundTransport.transport === 'http') {
      ...
      // Credo binds the shared app during initialize(), so wrap listen() to observe its server and bind errors.
      const originalListen = publicApp.listen.bind(publicApp)
      publicApp.listen = ((...args: unknown[]) => {
        const server = originalListen(...(args as never[]))
        publicServerBinding = new Promise<void>((resolve, reject) => {
          const onListening = () => {
  ...
  try {
    ...
    if (publicServerBinding) await publicServerBinding
  } catch (error) {
