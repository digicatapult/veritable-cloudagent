import { type AskarWallet } from '@credo-ts/askar'
...
export class DidWebDocGenerator {
  ...
  async generateAndRegister(
    ...
    try {
      ...
      const wallet = this.agent.context.wallet

      await this.testEncrypt(wallet as AskarWallet, generated.publicEncryptionKey)
    } catch (error) {
  ...
  public async testEncrypt(wallet: AskarWallet, publicKey: string) {
    const recipientKey = new Key(TypedArrayEncoder.fromBase64(publicKey), KeyType.X25519)
    const data = TypedArrayEncoder.fromString('test')

    const jwe = await wallet.directEncryptCompactJweEcdhEs({
      recipientKey,
      encryptionAlgorithm: 'A256GCM',
      data,
      header: {},
    })
    const decrypted = await wallet.directDecryptCompactJweEcdhEs({ compactJwe: jwe, recipientKey })
