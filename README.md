...
At startup, `did:web` server loads all DID `.json` files in `DID_WEB_DIR` (defaults to `/dids`). DID documents are saved to postgres, primary keyed by DID, and if a new document is uploaded with the same DID as an existing one, the new document overwrites. Apart from checking for an `id` key, the server doesn't validated the DID, so it's up to the user to load valid DIDs (make sure to match the domain in the DID with the domain of the `did:web` server).
...
- `GET '/did.json'` maps to `did:web:example.com`. If the database contains a row where `DID==did:web:example.com`, the document will be returned.
- `GET '/.well-known/did.json'` also maps to `did:web:example.com`. `.well-known` is a special case as part of the `did:web` spec.
...
| POSTGRES_PORT                    | N        | 5432                                                                       | If type of storage is set to "postgres" a port for the database needs to be provided                                               |
...
| POSTGRES_DB                      | N        | "did-web-server"                                                           | Name of the database for Postgres                                                                                                  |
...
| DID_WEB_DIR                      | N        | "/dids"                                                                    | Directory containing did:web documents                                                                                             |
