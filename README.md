...
The REST API is served by the private listener (port 3000 by default). The public listener serves DIDComm HTTP at `/didcomm`; WebSocket upgrades at `/didcomm-ws` use that same listener and port (5002 by default). Requests received on the public listener before startup completes receive `503 Service Unavailable`. The OpenAPI schema is available through SwaggerUI at <http://localhost:3000/swagger> and `/api-docs`.
...
- `DID_WEB_SERVICE_ENDPOINT=https://yourdomain.com/didcomm` - The service endpoint URL. This is embedded as the DIDComm serviceEndpoint in the generated did:web document, so it must include the `/didcomm` path DIDComm HTTP now listens on.
...
| ENDPOINT                                    | Y        | ['http://localhost:5002/didcomm', 'ws://localhost:5002/didcomm-ws']                                                                                                                                    | An array of endpoint for the agent app, if passing as an `environment` variable in docker, please pass as a comma delimited string. DIDComm HTTP/WS now bind to `/didcomm` and `/didcomm-ws` on the shared public listener. |
...
| INBOUND_TRANSPORT                           | Y        | "[{"transport": "http", "port": 5002}, {"transport": "ws"}]"                                                                                                                                             | Specifies the inbound transports as a JSON-parseable string. The WS transport shares the HTTP listener via an upgrade at `/didcomm-ws`; it does not bind a separate port. |
...
| OPENID4VC_ENABLED                           | N        | false                                                                                                                                                                                                   | Enables OpenID4VC.                                                                                                                |
| OPENID4VC_HOLDER_ENABLED                    | N        | false                                                                                                                                                                                                   | Enables the OpenID4VC holder role.                                                                                                 |
| OPENID4VC_ISSUER_ENABLED                    | N        | false                                                                                                                                                                                                   | Enables the OpenID4VC issuer role.                                                                                                |
| OPENID4VC_VERIFIER_ENABLED                  | N        | false                                                                                                                                                                                                   | Enables the OpenID4VC verifier role.                                                                                              |
| OPENID4VC_PUBLIC_BASE_URL                   | N        | ""                                                                                                                                                                                                      | Public base URL used by OpenID4VC. Development default: `http://localhost:5002`.                                                 |
| OPENID4VC_ID                                | N        | ""                                                                                                                                                                                                      | Public issuer and verifier resource identifier. Development default: `alice`.                                                     |
